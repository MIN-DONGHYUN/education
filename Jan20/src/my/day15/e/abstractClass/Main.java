...
public class Main {
	...
	public static void main(String[] args) {
		...
		//Animal은 미완성 클래스(==추상 클래스, abstract class)이므로 
		//Animal 을 사용하여 객체 (인스턴스)를 생성할 수 없다.!!!!!
		//but 저장소 역할은 할 수 있다.
		
		Animal ani2 = new Dog();
		// 자식 클래스로 생성되어진 객체(인스턴스)를 저장하는 용도로는 사용가능하다.
		*/
		...
		for(int i=0; i<aniArr.length; i++)		//배열만큼 반복
		{
			if(aniArr[i] != null)				// null이 아니라면 실행 
			{
				aniArr[i].cry(); 			// 동물 소리 메소드를 불러온다.
											// 만약 부모클래스에 cry가 없다면 오류가 생긴다.
											// 즉 모든 클래스에 오버라이딩을 한다면 빈 껍대기라도 부모클래스에 만들어줘야한다.
			}
		}
