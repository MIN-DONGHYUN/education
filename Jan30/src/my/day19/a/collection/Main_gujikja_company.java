...
public class Main_gujikja_company {
	...
	public static void main(String[] args) {
		...
		List<Member> mbrList = new ArrayList<>();   // 리스트에 맴버를 저장한다. 배열대신 사용
		...
		do {
			...
			switch (smenuNo) {
			case "1":			// 구직자 회원가입 
				Ctrl_total.register_gu(sc , mbrList);		//메소드 호출 (키보드 사용을 위해 Scanner 넘겨줌, 생성된 객체를 저장시키기 위해 배열을 넘겨줌)
												// guArr저장소를 넘겨줌 
				break;
			case "2":			// 구인회사 회원 가입   
				Ctrl_total.register_cp(sc , mbrList);		//메소드 호출 (키보드 사용을 위해 Scanner 넘겨줌, 생성된 객체를 저장시키기 위해 배열을 넘겨줌)
												// cpArr저장소를 넘겨줌
				break;
			case "3":			// 구직자 로그인
				Gujikja login_gu = Ctrl_total.login_gu(sc, mbrList);			//메소드 호출 (키보드 사용을 위해 Scanner 넘겨줌, 생성된 객체를 저장시키기 위해 배열을 넘겨줌)
																		// guArr저장소를 넘겨줌
				if(login_gu != null)			// login_gu의값이 null이 아니면  
				{
					System.out.println(">> 구직자 로그인 성공 ^^ << \n");
					
					Ctrl_total.gu_munu(sc,login_gu, mbrList);		 // 구직자 전용 메뉴
				...
				break;
			case "4":			// 구인회사 로그인  
				Company login_cp = Ctrl_total.login_cp(sc, mbrList);		// 메소드 호출 (키보드 사용을 위해 Scanner 넘겨줌, 생성된 객체를 저장시키기 위해 배열을 넘겨줌)
												// cpArr저장소를 넘겨줌 
				if(login_cp != null)			// login_gu의값이 null이 아니면  
				{
					System.out.println(">> 구직자 로그인 성공 ^^ << \n");
					
					Ctrl_total.cp_munu(sc,login_cp, mbrList);		 // 구인회사 전용 메뉴
																 //메소드 호출 (키보드 사용을 위해 Scanner 넘겨줌, 생성된 객체를 저장시키기 위해 배열을 넘겨줌)
