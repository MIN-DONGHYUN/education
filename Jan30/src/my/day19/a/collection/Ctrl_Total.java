...
public class Ctrl_Total implements Inter_Ctrl_Total {
	...
	@Override
	public void save_mbrList(Member mbr, List<Member> mbrList)	// 구직자든 구인회사인든 모든것을 사용하기 위해서는 부모클래스인 Member을 사용하는 것이다. 
	{
		Gujikja gu = null;
		Company cp = null;
		...
		if( cp == null && gu != null && 			// cp,gu, 구직자의 아이디, 비밀번호, 이름, 주번이 null이 아니면 
				...
				gu.getJubun() != null )
		{
			//mbrList[mbrList.size()++] = gu;			// 구직자배열에 넣어준다.
			mbrList.add(gu);		// 배열대신 List사용 
		...
		//구인회사부분
				...
				cp.getSeedMoney() > 0 )
		{
			//mbrList[mbrList.size()++] = cp;			// 구인회사배열에 넣어준다.
			mbrList.add(cp);			// 배열대신 List사용 
	...
	} //end of public void save_mbrList 
	...
	public void register_gu(Scanner sc, List<Member> mbrList) {
		...
		do
		{
			...
			if(!id.trim().isEmpty() )		// 공백제거하고 값이 비어있지 않는다면 
			{
				...
				{
					if( mbrList.get(i) instanceof Gujikja && id.equals(mbrList.get(i).getId()))		// 실제로 저장되어진 것을 가져왔을때 구직자 일경우 와 아이디가 같다면 실행
					{
		...
		save_mbrList(gu, mbrList);	// 불러온다.(메소드 호출)
	...
	public void register_cp(Scanner sc, List<Member> mbrList) {
		...
		// 아이디는 필수 입력사합이면서 중복된 아이디로 입력하면 안된다.!!
		// 입력받은 아이디가 중복된 아이디가 아닐때 까지 List<Member> mbrList반복해야 한다.
		...
		do
		{
			...
			if(!id.trim().isEmpty() )		// 공백제거하고 값이 비어있지 않는다면 
			{
				for(int i = 0; i < mbrList.size(); i++)
				{
					if( mbrList.get(i) instanceof Company && id.equals(mbrList.get(i).getId()))		// 실제로 저장되어진 것을 가져왔을때 구직자 일경우 와 아이디가 같다면 실행
					{
						System.out.println(">>이미 사용중인 아이디 입니다 << \n");
						isUseId = false;		// 무한 반복 다시 시작하기 위해 표식 남김 
						break;					// 아이디가 같으므로 더 검사할 필요가 없다.
		...
		save_mbrList(cp, mbrList);	// 불러온다.(메소드 호출)
	...
	public Gujikja login_gu(Scanner sc, List<Member> mbrList) {
		...
		for(int i=0; i<mbrList.size(); i++)					// 구직자 회원수만큼 반복 
		{
			if(mbrList.get(i) instanceof Gujikja && 			// 실제로 저장되어진 것을 가져왔을때 구직자 일경우
			   id.equals(mbrList.get(i).getId()) &&
			   passwd.equals(mbrList.get(i).getPasswd()))
			{
				return (Gujikja) mbrList.get(i);			// 구직자로 형변환후 리턴 
			}
		}
	...
	// == 구직자 전용메뉴 메소드 생성하기 == 
	@Override
	public void gu_munu(Scanner sc, Gujikja login_gu, List<Member> mbrList){
	...
	private void show_all_companyInfo(List<Member> mbrList) {
		...
		for(int i=0; i<mbrList.size(); i++)
		{
			if(mbrList.get(i) instanceof Company)// company에 있는 인스턴스 인가??
			{
	...
	public Company login_cp(Scanner sc, List<Member> mbrList) {
		...
		for(int i=0; i<mbrList.size(); i++)					// 구인회사 회원수만큼 반복 
		{
			if(mbrList.get(i)instanceof Company && 			// 실제로 저장되어진 것을 가져왔을때 구인회사 일경우
			   id.equals(mbrList.get(i).getId()) &&
			   passwd.equals(mbrList.get(i).getPasswd()))
			{
				return (Company) mbrList.get(i);			// 구인회사로 형변환후 리턴 
			}
		}
		return null;		//if문이 실행 안된다면 null값으로 리턴 
		
		
	} // end of public Company login_cp(Scanner sc, Member [] mbrList) {
	
	// == 구인회사 전용메뉴를 생성해주는 메소드 생성하기 == //
	@Override
	public void cp_munu(Scanner sc, Company login_cp, List<Member> mbrList) {
	...
	private void search_gender(Scanner sc, List<Member> mbrList) {
		...
		do { 
			...
			if( "남".equals(gender) || "여".equals(gender) ) { 
				...
				for(int i=0; i<mbrList.size(); i++) {				// 맴버수만큼 반복
					if(mbrList.get(i) instanceof Gujikja &&			// 구직자에 있는 것이며 성별이 구직자 성별과 같다면 
				...
				if(gender_count == 0) {							// 성별 수가 0이면은 
					...
					for(int i=0; i<mbrList.size(); i++) {				// 맴버수만큼
						if( mbrList.get(i) instanceof Gujikja &&		// // 구직자에 있는 것이며 성별이 구직자 성별과 같다면
								gender.equals( ((Gujikja) mbrList.get(i)).getGender() ) ) {

							String temp_info = mbrList.get(i).view_info();	// 구직자를 보여줘라 
		...
		do { 
			...
			try {
				...
				if(ageline < 0) {									// 0보다 작다면 
					...
					for(int i=0; i<mbrList.size(); i++) {				//맴버수만큼 반복
						if(mbrList.get(i) instanceof Gujikja &&			// 구직자에 있으면서 나이가 검색할 연령대와 같다면 
...
/*
		...
		{
			int ageLine = ((Gujikja) mbrList).getAge()/10*10;	//연령대를 ageLine에 대입 

			if(search_ageLine == ageLine && str_gender.equals(str_gender))	// 찾고자하는 연령대와 성별이 같습니까?
			{
				sb.append(((Gujikja)mbrList).view_info() + "\n");		// 누적한다.
