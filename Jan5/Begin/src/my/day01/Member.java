...
public class Member {
	...
	*/
	...
	// 2) static 변수 
	//staitc 변수는 동일한 클래스로 생성되어진 서로 다른 instance(객체)를 끼리 공유하는 변수이다.
	//static 변수를 class 변수라고도 부른다.
	static String address; //특정 인스턴트 안에 들어가는게 아닌 RAM에 개별적으로 나옴
	                       // 글자가 올바른것은 인스턴스, 글자가 기운것은 스테틱 변수  
	
	//  >>instance(인스턴스변수) 와 static  변수를 합쳐서 부를때 member(구성원)변수라고 부른다.<< 
